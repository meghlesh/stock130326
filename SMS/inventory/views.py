...
def sales_order_list(request):
    ...
    # BASE QUERY
    orders = SalesOrder.objects.filter(company=company)
    ...
    # 4-3-26: enhanced search to include order number as well
    customer = request.GET.get("customer")

    if customer:
        orders = orders.filter(
            Q(customer_name__icontains=customer) |
            Q(order_number__icontains=customer)
        )
    ...
    # =====================================================
    # ✅ PAGINATION ADDED (4-3-26)
    # =====================================================
    from django.core.paginator import Paginator

    orders = orders.order_by("-created_at")  # keep your ordering
    ...
    return render(
        ...
        {
            "orders": page_obj,  # ✅ send paginated orders instead of full queryset
            "page_obj": page_obj,  # ✅ used for pagination buttons in template
            "company": company,
            "total_orders": total_orders,
            "pending_orders": pending_orders,
            "completed_orders": completed_orders,
            "revenue": revenue,
            "selected_customer": customer,  # optional (for UI highlight)
        }
    )
...
def purchase_order_list(request):
    profile = UserProfile.objects.get(user=request.user)
    company = profile.company  # ✅ added company profile
    ...
    orders = (
        ...
        .annotate(
            status_order=Case(
                ...
                When(status="RECEIVED", then=4),  # RECEIVED should be last
                default=5,
    ...
    return render(
        ...
        {
            "orders": orders,
            "company": company,  # ✅ available for header
        }
    )
...
def add_purchase_order(request):
    ...
    if request.method == "POST":
        ...
        # 🔴 DUPLICATE BATCH VALIDATION BEFORE CREATING ORDER
    ...
    batch_number__iexact=batch_number
        ...
        # ✅ NOW create Purchase Order (only if validation passed)
        ...
        for index, product_id in enumerate(product_ids):
            ...
            # 🔒 Mandatory validation for expiry products
...
def receive_purchase_order(request, pk):
    ...
    for item in order.items.select_related("product"):
        ...
        if product.is_expiry_tracked:
            ...
            expiry_date = date(2099, 12, 31)  # safe dummy expiry
...
def batch_stock_list(request):
    ...
    company = profile.company   # ✅ ADD THIS

    status = request.GET.get("status")
    today = date.today()
    near_limit = today + timedelta(days=30)

    # ✅ BASE QUERY: ONLY NON-ZERO BATCHES
    batches = ProductBatch.objects.filter(
        company=company,
        quantity__gt=0
...
def product_stock_breakdown(request, pk):
    profile = UserProfile.objects.get(user=request.user)
    product = get_object_or_404(Product, id=pk, company=profile.company)
...
def adjust_batch_stock(request):
    ...
    if request.method == "POST":
        ...
        return redirect("batch_stock_list")
...
def add_vendor(request):
    ...
    if request.method == "POST":
        ...
        # ---------- COMPANY NAME ----------
        if not company_name:
            errors["company_name"] = "Vendor name is required."
        elif len(company_name) > 50:
            errors["company_name"] = "Vendor name cannot exceed 50 characters."
        elif not re.match(r'^[A-Za-z][A-Za-z\s&.\-]*$', company_name):
            errors["company_name"] = "Vendor name must contain valid characters."
        elif Vendor.objects.filter(
            company=company,
        ...
        if not display_name:
            ...
            errors["display_name"] = "Display name cannot exceed 50 characters."
        elif not re.match(r'^[A-Za-z][A-Za-z\s&.\-]*$', display_name):
            errors["display_name"] = "Display name must contain valid characters."
        elif Vendor.objects.filter(
            company=company,
            display_name__iexact=display_name
        ).exists():
            errors["display_name"] = "Vendor with this display name already exists."
    ...
    return render(request, "inventory/add_vendor.html", {"company": company})
...
def customer_edit(request, pk):
    ...
    AddressFormSet = inlineformset_factory(Customer, Address, form=AddressForm, extra=1, can_delete=True)
    ContactFormSet = inlineformset_factory( Customer, ContactPerson, form=ContactPersonForm, extra=1, can_delete=True)
    ...
    if request.method == "POST":
        ...
        contact_formset = ContactFormSet(instance=customer, prefix="contacts")
...
def reorder_suggestions(request):
    ...
    # ✅ PAGINATION (ISSUE #4 FIX)
    paginator = Paginator(suggestions, 10)  # 10 rows per page
    page_number = request.GET.get("page")
    page_obj = paginator.get_page(page_number)
    ...
    return render(
        ...
        {
            "suggestions": page_obj,  # 👈 IMPORTANT
            "company": company,
            "days": days,
        }
    )
...
def abc_inventory_classification(request):
    ...
    product_list.sort(key=lambda x: x["value"], reverse=True)

    total_value = sum([p["value"] for p in product_list])

    running_value = 0

    for item in product_list:

        running_value += item["value"]

        percent = (running_value / total_value) * 100 if total_value else 0

        if percent <= 70:
            item["product"].abc_class = "A"

        elif percent <= 90:
            item["product"].abc_class = "B"

        else:
            item["product"].abc_class = "C"
    ...
    return render(
        ...
        {
            "products": Product.objects.filter(company=company).order_by("abc_class"),
            "company": company
        }
    )
